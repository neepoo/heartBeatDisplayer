...
Windows 上的轻量心率悬浮工具：蓝牙直连 Garmin Forerunner 255，在桌面或无边框游戏上方显示当前 BPM 和最近 5 分钟的真实心率趋势。

## 开始使用

1. 解压 `HeartBeat-win-x64.zip`，保留压缩包内的全部文件，双击 `HeartBeat.exe`。发布包自带 .NET 运行时，不需要另行安装开发环境。
...
首次默认允许拖动，之后记住位置、锁定状态、背景透明度及最后成功连接的真实设备。重启后尝试连接上次的手表；主动点击“断开”会停止本次会话的重连。
...
- **游戏中看不到**：检查游戏是无边框模式，使用快捷键显示或托盘重置位置；独占全屏不在第一版支持范围，其他置顶窗口也可能影响显示顺序。
...
```powershell
# 核心行为测试；失败返回非零退出码
dotnet run --project tests/HeartBeat.Tests

# 真实 WPF 应用集成检查，会短暂显示本程序窗口并自动退出
# 使用独立目录，不读取或覆盖正常用户设置
dotnet run --project tests/HeartBeat.App.Tests -- --demo --settings-dir artifacts/smoke-session
...
输出：`artifacts\HeartBeat-win-x64\HeartBeat.exe` 和 `artifacts\HeartBeat-win-x64.zip`。

## 验证范围

核心测试覆盖协议解析、缓存、超时、重连与旧回调隔离；WPF 检查覆盖模拟数据流、穿透样式、隐藏/显示、不抢焦点、恢复位置、设置和退出，并输出预览图片。

真实手表连接、通知到屏幕延迟、多屏混合缩放及具体游戏中持续运行，需要在目标硬件上验证。请按源码包的 `docs/ble-validation.md` 执行实机检查。第一版不包含独占全屏注入、OBS 专用输出、历史导出和心率报警。
